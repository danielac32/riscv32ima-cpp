...
*/
    ...
    #define MINIRV32_STORE4(ofs, val) *reinterpret_cast<uint32_t*>(image + ofs) = val
    #define MINIRV32_STORE2(ofs, val) *reinterpret_cast<uint16_t*>(image + ofs) = val
    #define MINIRV32_STORE1(ofs, val) *reinterpret_cast<uint8_t*>(image + ofs) = val
    #define MINIRV32_LOAD4(ofs) *reinterpret_cast<uint32_t*>(image + ofs)
    #define MINIRV32_LOAD2(ofs) *reinterpret_cast<uint16_t*>(image + ofs)
    #define MINIRV32_LOAD1(ofs) *reinterpret_cast<uint8_t*>(image + ofs)
#endif
                ...
                case 0b1110011: // Zifencei+Zicsr
                {
                    ...
                    if( (microop & 3) ) // It's a Zicsr function.
                    {
                        ...
                        /*case 0x333:
                            ...
                            break;*/
                        //case 0x302: rval = 0; break; //pmpaddr0
                        ...
                        else
                        {
                            ...
                            case 0:  // ECALL (Environment Call)
                            {
                                ...
                                if ((syscall_code & 0xFFFF0000) == 0xcafe0000)  // Comprobar si la syscall tiene el prefijo 0xcafe
                                {
                                    ...
                                    //char *filename = (char *)REG(10); // El primer argumento: "/riscv/out.elf"
                                    //char *mode = (char *)REG(11);     // El segundo argumento: "rb"
                                    
                               
 
                                   /* printf("%x\n",REG(10) );
                                    uint32_t offset=REG(10);
                                    char c=MINIRV32_LOAD1(offset);

                                    while((c!=0)){
                                        printf("%c",c);
                                        offset++;
                                        c=MINIRV32_LOAD1(offset);
                                    }*/

                                    //while()
                                    //printf("%p\n",REG(10) );
                                    
                                    //printf("Syscall fopen: filename='%s', mode='%s'\n", filename, mode);


                                    MINIRV32_RETURNSYSCALL_HOST(code,state->regs[10],state->regs[11],state->regs[12],state->regs[13],state->regs[14],state->regs[15],state->regs[10]);
                                }
                                else
                                {
